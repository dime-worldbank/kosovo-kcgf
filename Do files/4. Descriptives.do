	...
	*Which are the firms' characteristics that increase their probability of having an approved loan?
	{
		...
		*Probability of loan
		reg 		has_loan 	 number_loans_last5years    												///
					productivity_r  lag1_productivity_r	sq_productivity_r 									///
		...
		*Figures
		foreach variable in 1 2 3 4  {
				...
				if `variable' == 1   local xtitle = "Number  of employees (2018)"
				
				if `variable' == 2   local xtitle = "Deciles of sales per employee (2018)"

				if `variable' == 3   local xtitle = "Deciles of total sales (2018)"	

				if `variable' == 4   local xtitle = "Number loans in the last 5 years (2018)"		
				...
				///
				legend(order(1 "95% CI"  2 "Probability of approved loan in 2018" ) region(lwidth(none)) cols(1) size(medium) position(0) bplacement(seast)) ///
				///
				ysize(5) xsize(7)	 ///
				///
	...
	*--------------------------------------------------------->>>
	**
	*Distribution in the probability of having a loan approved in 2018
	{
	use 	"$data\final\firm_year_level.dta" if active == 1 & period == 2018, clear
		...
		**
		psmatch2 has_loan i.group_sme i.municipalityid i.sectionid productivity_r employees avgrowth_productivity_r avgrowth_employees,  n(3) common ties	
	...
	*Firms' characteristics by size 2015
	{
		use "$data\final\firm_year_level.dta" if period == 2015 & active == 1, clear
			...
			**
			...
			using "$output\tables\Firms' characteristics by size_2015.xls", by(size_firm) sd sdbracket
	}
	*________________________________________________________________________________________________________________________________*
	...
	*________________________________________________________________________________________________________________________________*
	{
		...
		*--------------------------------------------------------------------------------------------------------------------------------*
			...
			**
			br		Product id average_loan_r loanamount_r irate_nominal collateralvalue_r duration shareloan_turnover economic_recovery				    if Product != "Total" & period == 2021
	...
	*________________________________________________________________________________________________________________________________*
	{
		...
		use "$data\inter\KCGF.dta" if period > 2020 & inlist(size_kcgf, 1,2,3), clear
			...
			**	
			iebaltab GuaranteePercentageRequested Maturity NominalInterestrate employees turnover_r productivity_r , format(%12.2fc) grpvar(group_period) save("$output/tables/Balance test regular and economic recovery package window.xls") 	rowvarlabels replace 
	}
	*________________________________________________________________________________________________________________________________*
